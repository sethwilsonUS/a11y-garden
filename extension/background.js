...
async function runScan({ appOrigin, mode }) {
  ...
  if (!response.ok) {
    ...
    let errorMessage = `Failed to save the hosted result (HTTP ${response.status}).`;
    ...
    if (errorText) {
      try {
        ...
        if (response.status === 404) {
          errorMessage =
            "Hosted result endpoint not found. Check the A11y Garden origin in the extension popup.";
        }
      }
    }
