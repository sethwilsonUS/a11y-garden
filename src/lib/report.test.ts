...
describe("generateMarkdownReport", () => {
  ...
  describe("platform tip section", () => {
    it("includes platform tip when both platform and platformTip are provided", () => {
      const md = generateMarkdownReport(
        makeReport({
          platform: "squarespace",
          platformTip: "In Squarespace, edit each block to add descriptive link text.",
        }),
      );

      expect(md).toContain("## Squarespace Tip");
      expect(md).toContain("In Squarespace, edit each block to add descriptive link text.");
    });

    it("uses the correct display name for each platform", () => {
      const md = generateMarkdownReport(
        makeReport({
          platform: "wordpress",
          platformTip: "Install the WP Accessibility plugin.",
        }),
      );

      expect(md).toContain("## WordPress Tip");
    });

    it("omits platform tip section when platformTip is undefined", () => {
      const md = generateMarkdownReport(
        makeReport({ platform: "shopify" }),
      );

      expect(md).not.toContain("Tip");
    });

    it("omits platform tip section when platform is undefined", () => {
      const md = generateMarkdownReport(
        makeReport({ platformTip: "Some tip" }),
      );

      expect(md).not.toContain("Tip");
    });

    it("omits platform tip section when both are undefined", () => {
      const md = generateMarkdownReport(makeReport());

      expect(md).not.toContain("Tip");
    });

    it("falls back to raw slug when platform is not in PLATFORM_LABELS", () => {
      const md = generateMarkdownReport(
        makeReport({
          platform: "some-unknown-cms",
          platformTip: "Unknown CMS advice.",
        }),
      );

      expect(md).toContain("## some-unknown-cms Tip");
    });

    it("appears after top issues and before violations by rule", () => {
      const md = generateMarkdownReport(
        makeReport({
          topIssues: ["Issue one"],
          platform: "squarespace",
          platformTip: "Squarespace advice here.",
          rawViolations: JSON.stringify([
            {
              id: "link-name",
              impact: "serious",
              description: "test",
              help: "Links need text",
              helpUrl: "https://example.com",
              nodes: [{ html: "<a>", target: ["a"] }],
            },
          ]),
        }),
      );
      ...
      const tipIdx = md.indexOf("## Squarespace Tip");
      const rulesIdx = md.indexOf("## Violations by Rule");

      expect(issuesIdx).toBeLessThan(tipIdx);
      expect(tipIdx).toBeLessThan(rulesIdx);
    });
  });
  ...
  describe("complete report with all sections", () => {
    it("includes all sections in correct order", () => {
      ...
      const issuesIdx = md.indexOf("## Top Issues");
      const tipIdx = md.indexOf("## WordPress Tip");
      const rulesIdx = md.indexOf("## Violations by Rule");
